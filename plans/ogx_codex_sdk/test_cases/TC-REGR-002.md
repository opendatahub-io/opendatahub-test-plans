...
PodDisruptionBudget (PDB) behavior remain intact after LlamaStack
...
**Test Steps**:
1. Verify initial state: one LlamaStack pod running, HPA active:
   ```bash
   oc get hpa -n <namespace>
   oc get pods -l app=llamastack -n <namespace>
   ```
2. Generate sustained load against LlamaStack /v1/chat/completions
...
5. Initiate a rolling update of the LlamaStack deployment:
   ```bash
...
**Expected Results**:
- HPA scales LlamaStack pods from 1 to 2+ when CPU exceeds 70%
- HPA reports currentReplicas matching or exceeding desiredReplicas
- During rolling update, PDB ensures at most 1 pod is unavailable
  at any point (`oc get pdb` shows allowedDisruptions >= 0)
