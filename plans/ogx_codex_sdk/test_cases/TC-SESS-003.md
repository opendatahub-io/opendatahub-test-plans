...
**Preconditions**:
- LlamaStack running on port 8321 with Memories API configured
...
**Validation**:
- Query PostgreSQL: `SELECT session_id, content FROM session_turns
  WHERE session_id IN ('uuid-a', 'uuid-b') ORDER BY session_id,
  turn_number;` confirms each session only contains its own turns
