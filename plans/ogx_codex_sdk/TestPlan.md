...
This test plan validates LlamaStack's ability to serve as a drop-in
...
- SSE streaming from LlamaStack's `/v1/chat/completions` endpoint
...
- PostgreSQL-backed session state storage leveraging LlamaStack's
...
- Multi-user session isolation at the LlamaStack application layer
  (handled by RHOAI platform ingress)
- Integration with proprietary coding agent SDKs (Claude Code, Cursor)
- Codex CLI permission model enforcement within LlamaStack
  (client-side responsibility)
- Multi-model routing based on task type (P2, not MVP)

### 1.3 Test Objectives

1. Verify that LlamaStack's `/v1/chat/completions` endpoint emits SSE
...
- **API Integration Testing** — validate LlamaStack's
...
- **Performance Testing** — measure streaming TTFT, latency per HTTP
  ...
  under load
- **Integration Testing** — verify LlamaStack-to-vLLM communication,
...
- **Regression Testing** — ensure existing `/v1/chat/completions`
  ...
  to LlamaStack API surface, preserved HPA/PDB functionality
...
- LlamaStack distribution version: v0.5.0+rhai0 (ODH fork)
...
- Network: ClusterIP services on port 8321 (LlamaStack), port 8000
...
|----------|--------|---------|----------|
| `/v1/chat/completions` (LlamaStack) | POST | OpenAI-compatible chat completions endpoint on port 8321; handles SSE streaming and tool-call responses | P0 |
...
| `/v1/chat/completions` (vLLM) | POST | Underlying inference endpoint on port 8000; LlamaStack proxies to this | P0 |
...
| LlamaStackDistribution CR | K8s CRD | Custom resource managed by llama-stack-k8s-operator; provisions LlamaStack instances | P1 |
| `config.yaml` (LlamaStack) | Config | Distribution config; must register Memories and Compaction providers | P1 |
...
|----|----------|-------------------|----------|
| TC-E2E-001 | Multi-step coding task — fix failing test | /v1/chat/completions (LlamaStack), /v1/chat/completions (vLLM), Memories API, PostgreSQL session store, Session ID parameter | P0 |
| TC-E2E-002 | Session recovery during coding task | /v1/chat/completions (LlamaStack), Memories API, PostgreSQL session store, Session ID parameter | P0 |
| TC-E2E-003 | Multi-tool file exploration workflow | /v1/chat/completions (LlamaStack), /v1/chat/completions (vLLM), Session ID parameter | P0 |
| TC-E2E-004 | Concurrent Codex CLI sessions | /v1/chat/completions (LlamaStack), PostgreSQL session store, Session ID parameter | P1 |

### 6.2 E2E Coverage Matrix

| Endpoint (from Section 4) | E2E Scenarios |
|----------------------------|---------------|
| `/v1/chat/completions` (LlamaStack) | TC-E2E-001, TC-E2E-002, TC-E2E-003, TC-E2E-004 |
...
via vLLM over HTTP (port 8000) and LlamaStack distribution service
...
- **Streaming TTFT latency** — p95 TTFT must be under 200ms on
  validated hardware (A100-80/H100); measure LlamaStack proxy
...
- **Large dataset behavior** — test conversation compaction at model
  context window limits (e.g., 128K tokens for Llama 3.3 70B);
  verify compaction policy activates correctly
...
- **LlamaStack service account permissions** — validate operator-
...
- **Tool execution permissions** — Codex CLI executes tools client-
  side; LlamaStack only routes tool definitions. Verify LlamaStack
...
- Gateway API HTTPRoute or ClusterIP service for Codex CLI endpoint
  access
- HPA (Horizontal Pod Autoscaler) for LlamaStack instances
- PodDisruptionBudget for availability during rolling updates
- NetworkPolicies for pod-to-pod communication (LlamaStack, vLLM,
  PostgreSQL)
- PVCs for persistent storage

### 9.2 Configuration

- `OPENAI_BASE_URL`: environment variable pointing to LlamaStack
  service endpoint (port 8321)
- `config.yaml`: LlamaStack distribution config with Memories API
...
|----------|------------|----------|
| `/v1/chat/completions` (LlamaStack) | TC-STREAM-001, TC-STREAM-002, TC-STREAM-003, TC-STREAM-004, TC-E2E-001, TC-E2E-002, TC-E2E-003, TC-E2E-004, TC-REGR-001 | |
...
| `config.yaml` (LlamaStack) | TC-INFRA-002 | |
