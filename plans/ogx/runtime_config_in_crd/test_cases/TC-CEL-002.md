...
# TC-CEL-002: overrideConfig + disabledAPIs rejected

**Objective**: Verify that a CR specifying both
`spec.overrideConfig` and `spec.disabledAPIs` is rejected
...
- OGXServer CRD includes CEL rules enforcing mutual
  exclusivity between overrideConfig and disabledAPIs
...
4. Confirm no CR was created:
   ...
   ```bash
   oc get ogxserver cel-test-002 -n tc-cel-002 \
...
- The error message indicates a CEL validation failure
  ...
  and `disabledAPIs`.
- No OGXServer CR named `cel-test-002` is created.
